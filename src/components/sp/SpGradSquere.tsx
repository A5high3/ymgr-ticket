...
export function SpGradationSquere({ squere }: any) {
  ...
  useEffect(() => {
    const widthAndHeight = +`${getRandomInt(6, 9)}${getRandomInt(0, 9)}`;
  ...
  return (
    <div
      style={{
        marginTop: 20,
        marginLeft: +`${marginLeft3digit}${marginLeft2digit}${marginLeft1digit}`,
        width: +`${squereSize}`,
        height: +`${squereSize}`,
        position: "absolute",
        border: "3px solid #c21500",
        borderImage:
          "linear-gradient(to right, rgba(59, 196, 241) 0%, rgba(202, 79, 146) 100%)",
        borderImageSlice: 1,
        zIndex: -1,
